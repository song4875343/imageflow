...
const specPanel=document.querySelector('.spec-panel'), panelToggle=document.querySelector('#panelToggle'), modelPanel=document.querySelector('#modelPanel'), outputPanel=document.querySelector('#outputPanel'), outputOverlay=document.querySelector('#outputOverlay'), outputFrame=document.querySelector('#outputFrame'), outputPreset=document.querySelector('#outputPreset'), outputRatio=document.querySelector('#outputRatio'), outputWidth=document.querySelector('#outputWidth'), outputHeight=document.querySelector('#outputHeight'); let outputDirectoryHandle;
function setParameterPanelVisible(visible, notifyParent=true){ specPanel.hidden=!visible; panelToggle.setAttribute('aria-pressed',String(visible)); panelToggle.textContent=visible?'隐藏参数':'显示参数'; if(notifyParent&&window.parent!==window)window.parent.postMessage({type:'roomspace-panel-visibility',visible:Boolean(visible)},'*'); }
...
panelToggle.addEventListener('click',()=>setParameterPanelVisible(specPanel.hidden));
window.addEventListener('message',event=>{ const data=event.data; if(data?.type==='roomspace-panel-visibility') setParameterPanelVisible(Boolean(data.visible),false); });
